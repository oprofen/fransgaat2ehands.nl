...
<form action="<?php echo JRoute::_('index.php?option=com_' . $this->nameComponent . '&view=' . $this->nameItems); ?>"
            ...
            <?php else : ?>
                <table class="table table-striped" id="<?php echo $this->nameItem . 'List'; ?>">
                    ...
                    <?php foreach ($this->items as $i => $item) :
                        ...
                        $canCreate = $user->authorise('core.create', 'com_' . $this->nameComponent);
                        ...
                        $canChange = $user->authorise('core.edit.state', 'com_' . $this->nameComponent . '.' . $this->nameItem . '.' . $item->id) && $canCheckin;
                        ?>
                        <tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
                            ...
                            <td class="center">
                                <div class="btn-group">
                                    <?php echo JHtml::_('jgrid.published', $item->state, $i, $this->nameItems . '.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
                                    >
                                    <?php
                                    // Create dropdown items
                                    $action = $archived ? 'unarchive' : 'archive';
                                    JHtml::_('actionsdropdown.' . $action, 'cb' . $i, $this->nameItems);
                            ...
                            <td class="has-context">
                                <div class="pull-left break-word">
                                    ...
                                    <?php if ($canEdit || $canEditOwn) : ?>
                                        <a class="hasTooltip"
                                           href="<?php echo JRoute::_('index.php?option=com_' . $this->nameComponent . '&task=' . $this->nameItem . '.edit&id=' . $item->id); ?>"
                            ...
                            <td class="nowrap small hidden-phone">
                                <?php  echo "&euro; " . (float)$item->selling_price; ?>
                            </td>
                            <td class="nowrap small hidden-phone">
                                <?php  echo JHtml::_('date', $item->created, "d F Y h:i a"); ?>
                            </td>
